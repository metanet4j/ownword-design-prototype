...
- [审阅页](../brand.html) 提供浅深背景、字标组合、小尺寸与界面应用。用户已要求应用，品牌资产记为 `approved`；主界面应用效果仍待审阅。
- [原型](../index.html) 的导航和公开身份卡复用主标，favicon 使用 16px 版本。启动、身份解析、创建与保存使用同一主标及呼吸光晕；减少动态效果时保持静态。启动图标先于 React 显示，沿用本机语言与主题偏好，不添加额外等待时间。
