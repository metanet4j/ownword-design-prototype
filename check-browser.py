...
try:
    ...
    click_action('public'); wait_page('public')
    expect('!!document.querySelector("[data-action=flip-card]")', 'The 3D card exposes a manual flip button')
    expect('!document.querySelector(".identity-sculpture").classList.contains("rotating")', 'Reduced motion stops automatic 3D rotation')
    call('focus', '[data-action=flip-card]'); call('press', 'Enter')
    wait_until('document.querySelector(".identity-sculpture").dataset.face === "back"', 'Enter flips the card with reduced motion')
    expect('document.activeElement.dataset.action === "flip-card" && document.querySelector(".plate-front").hasAttribute("inert")', 'Flip keeps keyboard focus and hides the other face')
    call('press', 'Space')
    wait_until('document.querySelector(".identity-sculpture").dataset.face === "front"', 'Space flips back to the front')
