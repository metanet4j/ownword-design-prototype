...
try:
    ...
    click('Cancel')
    expect('document.activeElement.textContent.trim() === "Connect Wallet"', 'Closing the dialog returns focus to its trigger')
    ...
    click('Back to My Identity'); wait_page('identity')
    ...
    pref('en', 'light')
    wait_until('Array.from(document.querySelectorAll("[role=\\"alert\\"]")).some(e=>e.textContent.includes("Preferences could not be saved"))', 'Unwritable storage is announced instead of failing silently', diagnostic='Array.from(document.querySelectorAll("[role=\\"alert\\"]")).map(e=>e.textContent)')
