...
try:
    ...
    inspect('review', True)
    click_action('copy')
    wait_until('document.querySelector("[data-copy-feedback]")?.dataset.copyFeedback === "copied" && document.querySelector("[data-copy-feedback]").textContent.trim().length > 0', 'Copy success feedback', diagnostic='(document.querySelector("[data-copy-feedback]")||{}).dataset.copyFeedback')
    ...
    click_action('copy')
    wait_until('document.querySelector("[data-copy-feedback]")?.dataset.copyFeedback === "copyFailed" && !!document.querySelector(".identifier code").textContent', 'Copy failure preserves identifier', diagnostic='(document.querySelector("[data-copy-feedback]")||{}).dataset.copyFeedback')
