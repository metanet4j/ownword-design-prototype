const ContentWorkspace = React.forwardRef(function ContentWorkspace({active, identity, locale, t, settings, setSettings, onConnect, onIdentity, failCopy, onAccountEvent}, ref) {
  ...
  function restoreStorage() {
    ...
    const loaded=M.load(); if (loaded.readFailure) return;
    const item=currentDraft(), restored=M.recoverDrafts(loaded,dataRef.current.drafts,saved.current);
    saved.current=new Map(loaded.drafts.map(d=>[d.id,d.content])); setData(restored);
    const recovered=restored.drafts.filter(d=>!saved.current.has(d.id));
    setMessage(recovered.length ? 'draftRecoveredBody' : '');
    console.info('[Ownword content] storage recovered',{unsavedDrafts:recovered.length});
    const replacement=item && recovered.find(d=>d.recoveredFrom===item.id && d.authorBapId===item.authorBapId);
    if (replacement) {lastHash.current='#/write/'+encodeURIComponent(replacement.id);history.replaceState(null,'',lastHash.current);setRoute({page:'write',id:replacement.id});}
  }
  ...
  }, [current?.id, current?.content, settings.storage, identity?.bapId, ready, data.readFailure]);
  React.useEffect(() => {
  ...
  function renderScreen() {
    ...
    return <section className="content-workspace" data-content-screen="content">
      ...
      {data.readFailure && <div className="draft-error" role="alert"><p>{t('draftReadFailed')}</p><Button data-action="storage-retry" onClick={restoreStorage}>{t('retry')}</Button></div>}
