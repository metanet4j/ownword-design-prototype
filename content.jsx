const ContentWorkspace = React.forwardRef(function ContentWorkspace({active, identity, locale, t, settings, setSettings, onConnect, onIdentity}, ref) {
  ...
  if (route.page === 'write') return <section data-content-screen="write"><Button variant="quiet" onClick={() => go('content')}>{t('contentBack')}</Button><h1 tabIndex="-1">{current ? itemTitle(current) : t('contentMissing')}</h1>{current && <Field id="content-source" label="Markdown" multiline value={current.content} onChange={value => setData(d => ({...d, drafts: d.drafts.map(item => item.id === current.id ? {...item, content: value, updatedAt: new Date().toISOString()} : item)}))} />}</section>;
  if (route.page === 'read') return <section className="content-reading" data-content-screen="read"><Button variant="quiet" onClick={() => go('content')}>{t('contentBack')}</Button><h1 tabIndex="-1">{record ? itemTitle(record) : t('contentMissing')}</h1>{record && <pre className="content-plain">{record.content}</pre>}</section>;
